...
HOST_PARAMS="-h $MYSQL_HOST --port $MYSQL_PORT -u $MYSQL_USER"
...
if [ -z "${AWS_ACCESS_KEY_ID}" ] || [ -z "${AWS_SECRET_ACCESS_KEY}" ]; then
    ...
    mysqldump $HOST_PARAMS $MYSQLDUMP_OPTIONS $MYSQL_DATABASE | gzip | s3cmd put - s3://$AWS_PATH/${CLIENT_ID}-$MYSQL_DATABASE-$(date +"%Y%m%d%H%M%S").sql.gz
else
    echo "[mysqlbackup] Using AWS credentials as parameters for s3cmd"
    mysqldump $HOST_PARAMS $MYSQLDUMP_OPTIONS $MYSQL_DATABASE | gzip | s3cmd --access_key=$AWS_ACCESS_KEY_ID --secret_key=$AWS_SECRET_ACCESS_KEY put - s3://$AWS_PATH/${CLIENT_ID}-$MYSQL_DATABASE-$(date +"%Y%m%d%H%M%S").sql.gz
fi
